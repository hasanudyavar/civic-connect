...
export default function AdminComplaintDetail() {
  ...
  async function loadData() {
    ...
    const { data: staff } = await supabase.from('profiles').select('id, full_name')
      .in('role', ['dept_staff', 'ward_supervisor']).eq('is_active', true).order('full_name');
    ...
    <DashboardShell>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link href="/admin/complaints" className="p-2 rounded-xl hover:bg-[var(--glass-bg)] text-[var(--outline)]"><ArrowLeft className="w-5 h-5" /></Link>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 rounded-xl bg-[var(--surface-container-highest)] border border-[var(--glass-border)]">
                        <p className="text-[10px] font-bold text-[var(--outline)] uppercase mb-1">AI Severity</p>
