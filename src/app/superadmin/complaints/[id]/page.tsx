...
export default function SuperAdminComplaintDetail() {
  ...
  async function loadData() {
    ...
    const { data: staff } = await supabase.from('profiles').select('id, full_name')
      .in('role', ['dept_staff', 'ward_supervisor', 'taluk_admin']).eq('is_active', true).order('full_name');
    ...
    <DashboardShell>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link href="/superadmin/complaints" className="p-2 rounded-xl hover:bg-[var(--glass-bg)] text-[var(--outline)]"><ArrowLeft className="w-5 h-5" /></Link>
